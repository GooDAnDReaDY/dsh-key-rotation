...
- 2026-09-16 — Notify helpers extracted to `lib/notify-events.js` (`notifySwitch`, `pushEvent`, `notifyExhaustion`); `index.js` re-exports the public pair. `lib/index.js` `apply()` remains a single cordis wiring unit (~920 lines) — further split deferred as higher-risk than value (#312).
