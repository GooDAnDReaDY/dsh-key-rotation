...
import { keyTail, isLoopbackAddress, isTrustedBridgeRequest, SWITCHABLE_MESSAGE_PATTERN, DEFAULT_SWITCH_CODES, isValidRef, pickNext, applyCooldown, recordFailure, recordSuccess, computeBackoff } from './pool.js';
    ...
    credentials.resolve = async (ref) => {
      ...
      for (let i = 0; i < pool.refs.length; i++) {
        ...
        if (hit && typeof hit.value === 'string' && hit.value.length > 0) {
          ...
          // success resets exponential counter for this key
    ...
    return (async function* () {
      ...
      for (let attempt = 0; attempt < pool.refs.length; attempt++) {
        ...
        try {
          ...
          if (pool.state.lastUsed) recordFailure(pool, pool.state.lastUsed, Date.now(), cooldownMs);
