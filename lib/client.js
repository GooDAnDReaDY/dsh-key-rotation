  ...
  factory: (require) => {
    ...
    function KeyRotationSection(props) {
      ...
      const settingsScope = React.useMemo(() => {
        try {
          // Compat (#18): core 0.1.6-alpha.2+ exposes `configForms` and dropped
          // `settingsScope`, while 0.1.5-rc.x still exposes only `settingsScope`.
          // Resolve whichever service the host actually provides, so the card
          // mounts on both. Hosts that offer neither stay on the null path below.
          if (props.ctx && props.ctx.configForms) {
            return props.ctx.configForms.get(NS);
          }
