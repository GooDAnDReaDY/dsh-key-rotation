...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // settings.plugin.item renders inside Settings -> Plugins -> Plugin settings
      // as a collapsible card. key MUST equal the settings namespace (NS),
      // otherwise the tab silently skips rendering it.
      const tMod = ctx.locale && ctx.locale.bind ? ctx.locale.bind(NS) : ((k) => k);
      ctx.slots.inject('settings.plugin.item', () => ctx.slots.register({
        ...
        const t = (props && props.t) || tMod;
        const [open, setOpen] = React.useState(false);
        return h('div', { style: { border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '10px', padding: '10px 12px', marginBottom: '10px' } },
          h('div', { onClick: () => setOpen(!open), style: { cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
            h('strong', null, t('title')),
            h('span', { style: { opacity: .6 } }, open ? '▾' : '▸'),
          ),
          !open ? h('p', { className: 'krot-hint', style: { margin: '4px 0 0' } }, t('cardDesc')) : null,
          open ? h('div', { style: { marginTop: '8px' } },
            h(KeyRotationSection, { ...props, locale: props.locale || 'en' })
          ) : null,
        );
      }));
    }
