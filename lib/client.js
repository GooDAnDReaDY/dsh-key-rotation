  ...
  factory: (require) => {
    ...
    function KeyRotationSection(props) {
      ...
      const settingsScope = React.useMemo(() => {
        ...
        // Newer cores expose configForms; older cores expose settingsScope.
        // Feature-detect each service independently so a broken/newer provider
        // cannot prevent the legacy provider from being used as a fallback.
        try {
        ...
        try {
          return ctx.settingsScope?.bind?.({ namespace: NS }) ?? null;
        } catch (_) {
          return null;
        }
      }, [props.ctx]);
      ...
      const save = () => {
        ...
        if (settingsScope && typeof settingsScope.mutate === 'function') {
          (async () => {
            try {
              const ops = Object.entries(draft).map(([k, v]) => ({
                op: 'set',
                path: [k],
                value: v,
              }));
              const accepted = await settingsScope.mutate(ops, state.revision);
              if (accepted === false) throw new Error('settings write rejected');
          ...
          (async () => {
            try {
              for (const [k, v] of Object.entries(draft)) {
                const accepted = await settingsScope.set(k, v);
                if (accepted === false) throw new Error('settings write rejected');
              }
              setState((s) => ({ ...s, status: 'ready', value: draft, error: '' }));
              setDraft(null);
