  ...
  function rotate(options, pool) {
    return (async function* () {
      ...
      const penalizeRef = (targetRef, errCode, errMsg) => {
        ...
        // "rate limit exceeded" and "deadline exceeded" are not evidence
        // of an exhausted billing quota. Keep those on ordinary cooldowns.
        const isQuota = errCode === 'QUOTA' || cls.code === 'QUOTA' ||
          /\b(?:insufficient[_\s-]+quota|quota(?:\s+is)?\s+(?:exhausted|exceeded)|exceeded\s+(?:your\s+)?(?:current\s+)?quota)\b/i.test(errMsg);
            ...
            try {
              for await (const chunk of inner) {
                ...
                if (chunk && chunk.type === 'finish') {
                  ...
                  // Error/abort terminals are not successful responses. After a
